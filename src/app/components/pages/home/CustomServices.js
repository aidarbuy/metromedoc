...
export default ({iconColor, hoverColor, titleColor}) => (
	<div className="flex-container" style={{marginTop: 0, marginBottom: 0}}>
		{data.map((item, index) => (
			<Paper className="custom-services-item" key={index}>
        <div>{item.icon}</div>
        <div>{item.name}</div>
        <p style={getStyles().p} dangerouslySetInnerHTML={{__html: item.desc}}/>
        <RaisedButton containerElement={<Link to={item.href}/>} label={item.button} secondary={true}/>
			</Paper>
		))}
	</div>
);